...
void Examine_ELF_File()
{
    ...
    printf("2. The data encoding scheme of the object file: %d\n",header->e_ident[EI_DATA]);
    ...
    Elf32_Shdr* sec = (Elf32_Shdr*)(start + header->e_shoff);
    ...
    Elf32_Phdr* program = (Elf32_Phdr*)(start + header->e_phoff); /* Program header table file offset */
    printf("9. The size of each program header entry:\n");
     for (int i=0; i<ProgramsNum; i++)
    {
        printf("program number: %d Size: %d\n" , i, program->p_filesz); /* Segment size in file */
    	program++;
    }
...
void printSectionName(){
    ...
    Elf32_Shdr *shdr = (Elf32_Shdr *)(start + ehdr->e_shoff); /* Section header table file offset */
...
void printSymbol(){
   if (start == NULL)
    ...
    Elf32_Ehdr *ehdr = (Elf32_Ehdr*) start;
    Elf32_Shdr *shdr = (Elf32_Shdr *)(start + ehdr->e_shoff); /*start of section header*/
    unsigned int sectionsNum = ehdr->e_shnum; /* Section header table entry count */
    Elf32_Shdr *sh_strtab = &shdr[ehdr->e_shstrndx]; /* Section header string table index */
    const char *const sh_strtab_p = start + sh_strtab->sh_offset;
    ...
    for(int i = 0 ; i < sectionsNum ; i++)
    {
       if (shdr->sh_type == SHT_SYMTAB || shdr->sh_type == SHT_DYNSYM	) /* Symbol table  == */
        {
            Elf32_Sym *sym = (Elf32_Sym *)(start + shdr->sh_offset); /* Section file offset */
            int symbol_num = shdr->sh_size / shdr->sh_entsize; /* Entry size if section holds table */
            if(debugFlag){
                printf("Table Size: %d\n", sym->st_size);
            ...
            if(debugFlag)
                printf("    Name offset");
            printf("\n");
            Elf32_Shdr* symbol_names = (start + ehdr->e_shoff + sizeof(Elf32_Shdr) * shdr->sh_link); /* Link to another section  */
            int name_offset = symbol_names->sh_offset;
            for (int j = 0; j < symbol_num; j++)
            {
                printf("%-2d: %08x  %5d  %-10s %s",
                       j,
                       sym[j].st_value,
                       sym[j].st_shndx,
                       sh_strtab_p + shdr->sh_name, 
                       start + name_offset + sym[j].st_name);
                if(debugFlag)
                    printf("  %06d",sym[j].st_name);
            printf("\n");
            }
        }
        shdr++;
...
Elf32_Shdr* fetch_table(char* entryName, Elf32_Ehdr *elfHeader){
    Elf32_Shdr* string_table_entry = start+ elfHeader->e_shoff+(elfHeader->e_shstrndx*elfHeader->e_shentsize);
    for (size_t i = 0; i < elfHeader->e_shnum; i++){
        Elf32_Shdr* entry = start+elfHeader->e_shoff+(i* elfHeader->e_shentsize);
        char* name = start + string_table_entry->sh_offset + entry->sh_name;
...
/*To get the relation information regarding values, you need to first get the symbol's index 
(using ELF32_R_SYM on the info field of the relocation entry). 
You use that to index into the '.dynsym' table and get the symbol entry. The value is stored in that table.
...
void relocationTables(){
    ...
    Elf32_Ehdr *ehdr = (Elf32_Ehdr*) start;//the header
    Elf32_Shdr *shdr = (Elf32_Shdr *)(start + ehdr->e_shoff);  /*start of section header*/
    unsigned int sectionsNum = ehdr->e_shnum; /* Section header table entry count */
    Elf32_Shdr *sh_strtab = &shdr[ehdr->e_shstrndx]; /* Section header string table index */
    const char *const sh_strtab_p = start + sh_strtab->sh_offset;

    Elf32_Shdr * dynsym = fetch_table(".dynsym" , ehdr);
    Elf32_Shdr * dynstr = fetch_table(".dynstr" , ehdr);
    ...
    for(int i = 0 ; i < sectionsNum ; i++)
    {
       ...
       if (shdr->sh_type == SHT_REL)
        {   
            Elf32_Rel *relocationTables = (Elf32_Rel *)(start + shdr->sh_offset);
            int symbol_num = shdr->sh_size / shdr->sh_entsize;
            
            printf("Relocation section \'%s\' at offset %08x contains %d entry:\n", 
            sh_strtab_p + shdr->sh_name,
            shdr->sh_offset,
            symbol_num);
            ...
            for (int j = 0; j < symbol_num; j++)
            {
                Elf32_Sym* symbol_entry = start+dynsym->sh_offset+(ELF32_R_SYM(relocationTables[j].r_info)*sizeof(Elf32_Sym));
                int value = symbol_entry->st_value;
                char* symbol_name = start + dynstr->sh_offset+symbol_entry->st_name;
                printf(" %08x  %08x     %d       %08x      %s",
                       relocationTables[j].r_offset,
                       relocationTables[j].r_info,
                       ELF32_R_TYPE(relocationTables[j].r_info),
                       value,
                       symbol_name
                       ); 
                if(debugFlag)
                    printf("  %06d",relocationTables[j].r_info);
            printf("\n");
            }
        }
        shdr++;
