...
char *historyArr[HISTORY_MAX_SIZE];
int Counter = 0; //counter of commands
// int hisPoint = 0;
...
void addHistory(char* line_unparse){
    if(strncmp("!", line_unparse,1) == 0){return;}
    if(strcmp("history",line_unparse)==0){return;}
    // if(hisPoint >= HISTORY_MAX_SIZE){
    //     hisPoint = 0;
    // }

    historyArr[Counter%HISTORY_MAX_SIZE] = strdup (line_unparse);
    Counter++;
...
void printHistory(){
    int stamNter=0;
    if((Counter-1) >= HISTORY_MAX_SIZE){
        int current = ((int)(Counter-1)%HISTORY_MAX_SIZE);
        for(int j = current; j<HISTORY_MAX_SIZE ; j++ ){
            printf("%d : %s\n" ,stamNter, historyArr[j]);
            stamNter++;
        }
        for(int k = 0; k<current ; k++){
            printf("%d : %s\n" ,stamNter, historyArr[k]);
            stamNter++;
        ...
        for(int i = 0;i< Counter-1 ;i++){
            printf("%d : %s\n" ,stamNter, historyArr[i]);
            stamNter++;
        }
    }
//     int index=0;
//     if(currentHistoryCounter == HISTORY_MAX_SIZE){
//         for(int i=HISTORY_MAX_SIZE-1; i>=hisP;i--){
//             printf("%d : %s\n" ,index, historyArr[i]);
//             index++;

//     }
//    }
//    for(int i=hisP-1; i>=0; i--){
//         printf("%d : %s\n" ,index, historyArr[i]);
//         index++;
//    }
...
int main(int argc , char* argv[]){
    ...
    addHistory(buffer_user_command_holder);
    cmdLine * line = parseCmdLines(buffer_user_command_holder);
L2:
    if (strcmp(line->arguments[0], "cd") == 0) { cdHandler(line); }
    else if(strcmp(line-> arguments[0] , "quit") == 0) { freeProcessList(process_list); freeHistory(); exit(0); }
    ...
    else if(line->argCount == 1 && (strncmp(line-> arguments[0] , "!",1) == 0)){
        printf("hi\n");
        int indCall = -1;
        if(strncmp(line-> arguments[0]+1 , "!",1) == 0){
            indCall = (Counter-1)%HISTORY_MAX_SIZE;
            printf("!! is num : %d\n",indCall);
        }
        else{
            indCall = atoi(line->arguments[0]+1);
        }
        
        if (indCall < 0 || indCall > (Counter-1)%HISTORY_MAX_SIZE)
            fprintf(stderr,"Invalid out bund\n");
        else{
        
            line = parseCmdLines(historyArr[indCall]);
            goto L2;
        }
    }
